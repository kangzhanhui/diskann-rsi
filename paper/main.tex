...
\hypersetup{
  pdftitle={DiskANN-RSI: Query-Path Optimizations for a Rust-Native Vamana Index, and a Same-Machine Reproducibility Study},
  pdfauthor={Zhanhui Kang}
}
...
\author{
  Zhanhui Kang\\
  TerseMind\\
  \texttt{research@tersemind.ai} \quad \url{https://github.com/kangzhanhui}\\
  ORCID: \url{https://orcid.org/0009-0008-7791-1917}
}
...
repetitions per operating point, and we report the throughput recorded in the
frozen raw rows (per-repetition values are preserved in the logs).
...
cloud VM instance: 2 vCPU, 3.6\,GB RAM + 8\,GB swap, OpenCloudOS 9.4.
Dataset SIFT1M (1M $\times$ 128, L2); recall@10; 10k queries; 3 repetitions;
QPS is the throughput recorded in the frozen raw rows. Single-thread (1T) is
the reference protocol; NSG's search
...
Figure~\ref{fig:recallqps} shows the recall--QPS curves reconstructed from the
frozen raw measurements (each point is the median over repeats), and
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{fig_recall_qps.pdf}
\caption{Recall@10 versus QPS on SIFT1M, measured in one session on one
machine: hnswlib (master-default), stock Vamana, DiskANN-RSI and NSG (NSG at
two threads, all others at one). Each marker is the median over repeats at one
operating point of the raw rows in \texttt{bench/results/}; curves are
reconstructed by \texttt{bench/plots/recall\_qps.py}. The $\alpha{=}1.0$
DiskANN-RSI curve sits below stock Vamana at low search depths and converges to
it at high recall, while $\alpha{=}1.2$ closes most of that gap.}
\label{fig:recallqps}
\end{figure}
...
\bottomrule
\end{tabular}
\caption{Recall@10/QPS ladders on SIFT1M (1T; NSG 2T; QPS as recorded in the
frozen raw rows, 3 repetitions per operating point).}
\label{tab:ladder}
\end{table}
...
baseline used here. (ii) \emph{Hardware regime}: on this 2-vCPU host the
1M-vector set (512\,MB) and the graph stream from DRAM, while the 2--4\,MB
visited set is cache-resident under either stamp width, so halving the 16-bit
...
point) --- quantization pays when vector-fetch bandwidth is the bottleneck,
which it is not on this compute-bound host; \textbf{candidate-queue merge insertion}
...
The artifact is available at \url{https://github.com/tersemind/diskann-rsi}
under the MIT License. Code, \texttt{job.json} configurations, raw JSONL measurements
